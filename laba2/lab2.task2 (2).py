...
def shifr(array):
    ...
    for j in array:
        name_id = j
        if name_id in deque:
            index = deque.index(name_id) #нахождение индекса текущего символа
            shifr_index = (index + 2) % len(deque)#вычисление индекса символа для шифрования (сдвиг на 2 позиции по кругу).
